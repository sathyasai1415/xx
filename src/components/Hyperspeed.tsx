...
// postprocessing v6 — SMAAEffect.searchImageDataURL removed, preset still works
...
const distortions: Record<string, any> = {
  mountainDistortion: {
    ...
    getJS: (progress: number, time: number) => {
      const f = 0.02, uFreq = mountainUniforms.uFreq.value, uAmp = mountainUniforms.uAmp.value;
  ...
  xyDistortion: {
    ...
    getJS: (progress: number, time: number) => {
      const f=0.02, uFreq=xyUniforms.uFreq.value, uAmp=xyUniforms.uAmp.value;
      const d = new THREE.Vector3(
        Math.cos(progress*Math.PI*uFreq.x+time)*uAmp.x-Math.cos(f*Math.PI*uFreq.x+time)*uAmp.x,
        Math.sin(progress*Math.PI*uFreq.y+time+Math.PI/2)*uAmp.y-Math.sin(f*Math.PI*uFreq.y+time+Math.PI/2)*uAmp.y,0,
      );
    ...
    getDistortion: `uniform vec2 uFreq; uniform vec2 uAmp; #define PI 3.14159265358979
      vec3 getDistortion(float progress){
        ...
        return vec3(
          ...
          sin(progress*PI*uFreq.y+uTime)*uAmp.y-sin(f*PI*uFreq.y+uTime)*uAmp.y,0.
        );
      }`,
    getJS: (progress: number, time: number) => {
      const f=0.0125, uFreq=LongRaceUniforms.uFreq.value, uAmp=LongRaceUniforms.uAmp.value;
      const d = new THREE.Vector3(
        Math.sin(progress*Math.PI*uFreq.x+time)*uAmp.x-Math.sin(f*Math.PI*uFreq.x+time)*uAmp.x,
        Math.sin(progress*Math.PI*uFreq.y+time)*uAmp.y-Math.sin(f*Math.PI*uFreq.y+time)*uAmp.y,0,
      );
...
class HyperspeedApp {
  options: any; container: HTMLElement; renderer: THREE.WebGLRenderer;
  ...
  hasValidSize: boolean; renderPass!: RenderPass; bloomPass!: EffectPass;
  ...
  dispose(){
    this.disposed=true;
    this.scene.traverse((obj: any)=>{ if(!obj.isMesh) return; if(obj.geometry) obj.geometry.dispose(); if(obj.material){ if(Array.isArray(obj.material)) obj.material.forEach((m: any)=>m.dispose()); else obj.material.dispose(); } });
    this.scene.clear();
  ...
  setSize(w: number, h: number, s: boolean){ if(w<=0||h<=0){this.hasValidSize=false;return;} this.composer.setSize(w,h,s); this.hasValidSize=true; }
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    const distortionKey = (opts as any).distortion as string;
    (opts as any).distortion = distortions[distortionKey] ?? distortions.turbulentDistortion;
