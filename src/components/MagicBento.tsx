...
const ParticleCard = ({
  ...
  enableTilt = false, clickEffect = true, enableMagnetism = false, onClick,
}: ParticleCardProps) => {
...
const GlobalSpotlight = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }: GlobalSpotlightProps) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const cards = gridRef.current.querySelectorAll<HTMLElement>('.mb-magic-card');
      if (!mouseInside) {
    ...
    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll<HTMLElement>('.mb-magic-card').forEach(card => card.style.setProperty('--glow-intensity', '0'));
...
const MagicBento = ({
  ...
  const baseCardClass = `mb-magic-card ${textAutoHide ? 'mb-magic-card--text-autohide' : ''} ${enableBorderGlow ? 'mb-magic-card--border-glow' : ''}`;
          ...
          enableStars ? (
            ...
            >
              <div className="mb-card-header">
                <div className="mb-card-label" style={card.accent ? { color: card.accent } : {}}>{card.icon && <span className="mr-1">{card.icon}</span>}{card.label}</div>
              </div>
