  ...
  return (
    <footer className="relative w-full mt-16">
      <div className="max-w-6xl mx-auto px-5">
        <div className="rounded-3xl px-6 sm:px-10 py-10" style={{ background: 'rgba(15,12,25,0.92)', border: '1px solid rgba(139,92,246,0.15)', backdropFilter: 'blur(12px)' }}>
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-10">
            ...
            <div className="max-w-xs">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-white font-black text-xl tracking-tight">MiSlice</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-8">
              {Object.entries(links).map(([section, items]) => (
                <div key={section}>
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-violet-400 mb-3">{section}</h4>
                  <ul className="space-y-2">
                    {items.map(link => (
                      <li key={link.label}>
                        <button
                          onClick={link.action}
                          className="text-xs font-bold text-white/40 hover:text-white transition-colors text-left"
                        >
                          {link.label}
                        </button>
                      </li>
          ...
          <div className="border-t border-white/8 mt-10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-[11px] text-white/30 font-bold flex items-center gap-1">
            ...
            <div className="flex items-center gap-2">
              {SOCIALS.map(s => (
                <a
                  ...
                  href="#"
                  aria-label={s.label}
                  className="w-8 h-8 rounded-full flex items-center justify-center text-white/40 hover:text-violet-300 transition-colors"
